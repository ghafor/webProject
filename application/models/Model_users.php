...
class Model_users extends CI_Model
{
	...
	public function getUserData($start = null,$limit = null,$userId = null) 
	{
		if($userId) {
		...
		// $sql = "SELECT * FROM users WHERE id != ? ORDER BY id DESC LIMIT $limit,$offset";
		// $query = $this->db->query($sql, array(1));
		$this->db->select('*')->from('users')->limit($limit,$start)->order_by('id','DESC');
